...
export default {
  ...
  methods:{
    handleCommand(command) {
     ...
     release(){
       ...
       if(!this.insertId){
          this.$notify.error({title: '错误',message: '页面错误请刷新'});
       }else{
  ...
  mounted () {
    this.insertId = this.$route.query.id;
    if(this.insertId){
      console.log(this.insertId)
      this.$axios.get('/app/getArticleById/'+this.insertId).then(res => {
          let data=res.data.list[0];
          this.msg=data.title;
          this.type=data.typeName;
          this.typeid=data.typeid;
          this.jiajei=data.articleCointent;
          this.neir=data.introduce;
